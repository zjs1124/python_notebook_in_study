...
class CarSpider(scrapy.Spider):
    ...
    start_urls = ["https://www.che168.com/beijing/a0_0msdgscncgpi1ltocsp1exf4/?pvareaid=102179"]
    current_page = 1
    end_page = 21
    custom_settings = {
    ...
    def start_requests(self):
        url = "https://www.che168.com/beijing/a0_0msdgscncgpi1ltocsp1exf4/?pvareaid=102179"
        headers = {
    ...
    def parse(self, response):
        ...
        if self.current_page < self.end_page:
            ...
            next_url = f"https://www.che168.com/beijing/a0_0msdgscncgpi1ltocsp{next_page}exf4/?pvareaid=102179"
            self.current_page += 1
    ...
    def car_info_details_parse(self, response):
        ...
        for vehicle in vehicle_profile:
            gearbox = vehicle.xpath("ul[1]/li[3]/text()").get()# 变速箱
            fuel_type = vehicle.xpath("ul[1]/li[4]/text()").get() # 燃料类型
            cltc_pure_electricity_endurance_mileage = vehicle.xpath("ul[1]/li[5]/text()").get()# CLTC纯电续航里程
            release_time = vehicle.xpath("ul[1]/li[6]/text()").get()# 发布时间
            annual_inspection_expiration = vehicle.xpath("ul[2]/li[1]/text()").get()# 年检到期
            insurance_expiration = vehicle.xpath("ul[2]/li[2]/text()").get()# 保险到期
            warranty_expiration = vehicle.xpath("ul[2]/li[3]/text()").get()# 质保到期
            transfer_times = vehicle.xpath("ul[2]/li[5]/text()").get()# 过户次数
            standard_fast_charge = vehicle.xpath("ul[2]/li[7]/text()").get()# 标准快充
            engine = vehicle.xpath("ul[3]/li[1]/text()").get()# 发动机
            vehicle_level = vehicle.xpath("ul[3]/li[2]/text()").get()# 车辆级别
            vehicle_color = vehicle.xpath("ul[3]/li[3]/text()").get()# 车身颜色
            drive_mode = vehicle.xpath("ul[3]/li[4]/text()").get()# 驱动方式
            standard_capacity = vehicle.xpath("ul[3]/li[6]/text()").get()# 标准容量
            standard_slow_charge = vehicle.xpath("ul[3]/li[7]/text()").get()# 标准慢充
        ...
        configuration_highlights_text = '/'.join(configuration_highlights).strip()# 配置亮点
